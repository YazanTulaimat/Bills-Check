class e {
    ...
    static async listCameras(a = !1) {
        ...
        let b = async () => (await navigator.mediaDevices.enumerateDevices()).filter((d) => 'videoinput' === d.kind),
            c;
        try {
            a && (await b()).every((d) => !d.label) && (c = await navigator.mediaDevices.getUserMedia({ audio: !1, video: !0 }));
        } catch (d) {}
        try {
                ...
                if (((this._paused = !1), this.$video.srcObject)) await this.$video.play();
                    ...
                    try {
                        ...
                        !this._active || this._paused
                            ...
                            : (this._setVideoMirror(b), (this.$video.srcObject = a), await this.$video.play(), this._flashOn && ((this._flashOn = !1), this.turnFlashOn().catch(() => {})));
                    } catch (a) {
    ...
    static async scanImage(a, b, c, d, f = !1, h = !1) {
        let m,
            n = !1;
        b && ('scanRegion' in b || 'qrEngine' in b || 'canvas' in b || 'disallowCanvasResizing' in b || 'alsoTryWithoutScanRegion' in b || 'returnDetailedScanResult' in b)
            ? ((m = b.scanRegion), (c = b.qrEngine), (d = b.canvas), (f = b.disallowCanvasResizing || !1), (h = b.alsoTryWithoutScanRegion || !1), (n = !0))
            : b || c || d || f || h
                ...
                q = await new Promise((l, v) => {
                    ...
                    u = (t) => {
                        t.data.id === y &&
                            ...
                            clearTimeout(w),
                            null !== t.data.data ? l({ data: t.data.data, cornerPoints: e._convertPoints(t.data.cornerPoints, m) }) : v(e.NO_QR_CODE_FOUND));
                    };
                    r = (t) => {
                        g.removeEventListener('message', u);
                        g.removeEventListener('error', r);
                    ...
                    (async () => {
                        try {
                            ...
                            if (/not implemented|service unavailable/.test(g))
                                return (e._disableBarcodeDetector = !0), e.scanImage(a, { scanRegion: m, canvas: d, disallowCanvasResizing: f, alsoTryWithoutScanRegion: h });
    ...
    setGrayscaleWeights(a, b, c, d = !0) {
        e._postWorkerMessage(this._qrEnginePromise, 'grayscaleWeights', { red: a, green: b, blue: c, useIntegerApproximation: d });
    }
    setInversionMode(a) {
    ...
    _scanFrame() {
        !this._active ||
            this.$video.paused ||
            this.$video.ended ||
            ('requestVideoFrameCallback' in this.$video ? this.$video.requestVideoFrameCallback.bind(this.$video) : requestAnimationFrame)(async () => {
                    ...
                    c
                        ? (this._onDecode ? this._onDecode(c) : this._legacyOnDecode && this._legacyOnDecode(c.data),
                          this.$codeOutlineHighlight &&
                              ...
                              this.$codeOutlineHighlight.firstElementChild.setAttribute('points', c.cornerPoints.map(({ x: d, y: f }) => `${d},${f}`).join(' ')),
